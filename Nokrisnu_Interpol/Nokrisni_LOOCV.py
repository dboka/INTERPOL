...
# ============================================================
# LOOCV FOR NOKRIŠŅI — IDENTICAL STRUCTURE TO TEMPERATURE LOOCV
# ============================================================
def Kriging_LOOCV_Nokrisni_Config(
    ...
    df_grid["elevation"] = df_grid["h5"]     # <--- USE REAL GRID HEIGHT
    df_grid["gams"] = 0                       # <--- no P on grid → gams=0

    # ============================================================
    # CONFIG (SAME STYLE AS TEMPERATURE CONFIGS)
    # ============================================================
    configs = {
        1: {
    ...
    for i in range(len(df)):
        ...
        model = gs.Exponential(
         dim=2,
            ...
            nugget=0           # LVĢMC noteikts: tirradnis = 0
            )
...
df_out.to_csv(
    os.path.join(OUT_DIR, "LOOCV_nokrisni.csv"),
    index=False,
    encoding="utf-8-sig"
)
